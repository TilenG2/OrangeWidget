...
from .util.uncertainforestutil import UncertainForestLearner

class OWUncertainForest(OWBaseLearner):
    name = "Uncertain Forest"
    description = "Predict using an ensemble of decision trees."
    icon = "icons/UncertainForest.svg"
    replaces = [
    ...
    priority = 40
    keywords = "random forest, uncertain forest"

    LEARNER = UncertainForestLearner

    n_trees = settings.Setting(10)
    ...
    def create_learner(self):
        ...
        if self.class_weight:
            common_args["balanced_classes"] = True
...
if __name__ == "__main__":  # pragma: no cover
    WidgetPreview(OWUncertainForest).run(Table("iris"))
